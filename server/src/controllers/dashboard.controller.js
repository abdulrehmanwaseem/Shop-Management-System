...
const getDashboardData = TryCatch(async (req, res, next) => {
  ...
  const lowOnStockCount = Number(
    lowOnStockCountResult[0]?.lowonstockcount || 0
  );

  const totalExpense = parseInt(expense._sum.amount);
  const totalPurchase = purchase._sum;
  const totalSale = sales._sum;

  const totalRevenue = parseInt(revenue._sum.revenue) - totalExpense;
  const expectedRevenue = parseInt(totalSale.revenue);
  const previousRecievable =
    44190 - 11500 - 3480 - 500 - 5000 - 4500 - 3000 - 11560 - 2400 - 1000;
  // 200 discount
  const amountInCash = parseInt(capital?.amount || 0) + 2400 + 1000;
...
const getDashboardTablesData = TryCatch(async (req, res, next) => {
  ...
  const startDate = new Date(date[0] || Date.now());
  const endDate = new Date(date[1] || Date.now());
